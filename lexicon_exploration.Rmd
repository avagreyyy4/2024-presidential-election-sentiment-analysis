...
- negations: could use a basic lexicon then ass additional values + words

- good that the data is shorter to avoid potentiall zero-ing out 
...
# Use the tidy candidates data (group emotions by candidate) to analze specific emotions

# Here I used an example of fear
harris_fear <- tidy_candidates |>
  filter(candidate_mentioned == "Harris") |>
  inner_join(nrcfear) |>
  count(word, sort = TRUE)

trump_fear <- tidy_candidates |>
  filter(candidate_mentioned == "Trump") |>
  inner_join(nrcfear) |>
  count(word, sort = TRUE)

harris_fear |> head(10)
trump_fear |> head(10) # same trend that kamalas may have been more policy related while trump was more character or action related
```
...
```{r}
bing_word_count_trump <- tidy_candidates |> 
  filter(word != "trump", word != "vote") |>  
  inner_join(get_sentiments("bing"), by = "word") |>  
  count(candidate_mentioned, word, sentiment, sort = TRUE) |>  # Keep candidate_mentioned
  ungroup()|>
...
bing_word_count_harris <- tidy_candidates |> 
  filter(word != "trump", word != "vote") |>  
  inner_join(get_sentiments("bing"), by = "word") |>  
  count(candidate_mentioned, word, sentiment, sort = TRUE) |>  # Keep candidate_mentioned
  ungroup()|>
