...
```{r}
clean_data <- read_csv("my_output.csv")
```
```{r}
...
vader_word_score_trump <- tidy_candidates |>
  filter(candidate_mentioned == "Trump", word != "trump", word != "vote") |>
  group_by(word) |>
  summarise(avg_sentiment = mean(vader_score, na.rm = TRUE), count = n()) |>
  filter(count >= 5) |>
  arrange(avg_sentiment)  # Most negative at the top

# For Harris
vader_word_score_harris <- tidy_candidates |>
...
word_cloud_candidate <- function(candidate) {
  tidy_candidate <- tidy_candidates |>
    filter(candidate_mentioned == candidate, word != "trump") |>
    # Join VADER data
    inner_join(vader_data, by = "word") |>
    # Bin VADER scores into sentiment labels
